...
public class AnswerDAO {
	...
	public List<Answer> getAnswers(int formId) {
		...
		SQLiteDatabase db = DatabaseHandler.getInstance(context).getWritableDatabase();
		Cursor c = db.query(DatabaseHandler.TABLE_ANSWERS, null, Answer.COLUMN_FORM_ID+"= "+formId, null, null, null, null);
		try {
			if(c.getCount() > 0) {
				while(c.moveToNext()){
	...
	public List<String> getPatientsNames(List<Form> forms) {
		List <String> evaluatorNames = new ArrayList<String>();
		for(Form form: forms) {
			evaluatorNames.add(getEvaluatorName(form.getFormId()));
		}
