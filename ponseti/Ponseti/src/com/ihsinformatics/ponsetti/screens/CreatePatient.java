...
import com.ihsinformatics.ponsetti.view.Fragments.common.AddPhotosFragment;
...
public class CreatePatient extends FragmentActivity implements ActionBar.TabListener {
	...
	MyFragment generalInfoFragment, gardianInfoFragment, familyHistoryFragment, addPhotosFragment,
			...
			public void onClick(View v) {
				...
				if(informationgeneralInfoFragment != null) {
					...
					if(form == null) {
						...
						form = new FormDAO(CreatePatient.this).getForm(Form.COLUMN_FORM_ID+"="+i);
						// form.setFormId(i);
						isUpdatable = false;
					}
					...
					if(!isUpdatable) {
						...
						ArrayList<Bitmap> bitmapsList = ((AddPhotosFragment)addPhotosFragment).getPhotos();
						for(Bitmap bitmap: bitmapsList) {
							String photoContent = Tools.getInstance().getBitmapEncodedContent(bitmap);
							Photo photo = new Photo(photoContent, 0, form.getIcrId());
							new PhotoDAO(CreatePatient.this).insert(photo);
	...
	private void initFragments() {
		...
		args7.putInt(MyFragment.LAYOUT_ID, R.layout.fragment_visit_add_photo);
		...
		addPhotosFragment.setArguments(args7);
	...
	private void init() {
		...
		if(form != null){
			if(form.getIcrId().startsWith("OFF")) {
				btnSave.setVisibility(View.VISIBLE);
			} else {
				btnSave.setVisibility(View.GONE);
			}
			List<Answer> answers = new AnswerDAO(this).getAnswers(Answer.COLUMN_FORM_ID+"= "+form.getFormId(), null);
			generalInfoFragment.requestLoadData(answers);
			gardianInfoFragment.requestLoadData(answers);
			familyHistoryFragment.requestLoadData(answers);
			referalInformationFragment.requestLoadData(answers);
			diagnosisFragment.requestLoadData(answers);
			physicalInformationFragment.requestLoadData(answers);
			((AddPhotosFragment)addPhotosFragment).enableDataLoading(form);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		try {
			if(requestCode == Tools.REQUEST_TAKE_PHOTO) {
				try {
					Bitmap bitmap = BitmapHolder.getinstance().consumeBitmaps(getClass().getName()).get(0);
					((AddPhotosFragment) addPhotosFragment).addBitmap(bitmap);
					Toast.makeText(this, "Photo added", Toast.LENGTH_SHORT).show();
				} catch (BitmapConsumedException e) {
					Toast.makeText(this, e.getMessage(), Toast.LENGTH_SHORT).show();
					e.printStackTrace();
				}
			} else if(requestCode == DATE_OF_BIRTH_REQUEST) {
				((PatientGeneralInfoFragment) generalInfoFragment).setEtDateOfBirth(data.getStringExtra("date"));
			} else if(requestCode == DATE_OF_EVALUATION_REQUEST) {
				((PatientDiagnosisFragment) diagnosisFragment).setEtDateOfEvaluation(data.getStringExtra("date"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
