...
public class AddVisit extends FragmentActivity implements ActionBar.TabListener {
	...
	MyFragment generalInfoFragment, leftFootFragment, rightFootFragment, complicationsFragment, addPhotosFragment;
			...
			public void onClick(View v) {
				...
				if(informationGeneralInfoFragment != null) {
					...
					if(form == null) {
						...
						form = new FormDAO(AddVisit.this).getForm(Form.COLUMN_FORM_ID+"="+i);
						// form.setFormId(i);
						isUpdatable = false;
					}
					...
					if(!isUpdatable) {
						...
						for(Bitmap bitmap: bitmapsList) {
							...
							new PhotoDAO(AddVisit.this).insert(photo);
	...
	private void initFragments() {
		...
		args5.putInt(MyFragment.LAYOUT_ID, R.layout.fragment_visit_add_photo);
		...
		addPhotosFragment.setArguments(args5);
	}
	...
	private void init() {
		...
		if(form != null){
			...
			leftFootFragment.requestLoadData(answers);
			rightFootFragment.requestLoadData(answers);
			complicationsFragment.requestLoadData(answers);
			addPhotosFragment.requestLoadData(answers);
	...
	protected void onActivityResult(int arg0, int arg1, Intent arg2) {
		try {
			if(arg0 == Tools.REQUEST_TAKE_PHOTO) {
				try {
			...
			} else if(arg0 == DATE_OF_VISIT) {
				((VisitGeneralInfoFragment) generalInfoFragment).setEtDateOfVisit(arg2.getStringExtra("date"));
			}else if(arg0 == DATE_OF_NEXT_VISIT) {
				((VisitGeneralInfoFragment) generalInfoFragment).setEtDateOfNextVisit(arg2.getStringExtra("date"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
