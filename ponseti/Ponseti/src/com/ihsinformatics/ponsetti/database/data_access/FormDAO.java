...
public class FormDAO {
	...
	public long insert(Form form) {
	    ...
	    if(form.getIcrId() == null){
	    	...
	    	values.put(Form.COLUMN_TIME_STAMP, epoch);
		} else {
	...
	public String[] getIcrIds(String whereClauseCheck) {
		String[] icrIds;
		SQLiteDatabase db = DatabaseHandler.getInstance(context).getWritableDatabase();
		Cursor c = db.query(DatabaseHandler.TABLE_FORMS, null, whereClauseCheck, null, null, null, null);
		
		icrIds = new String[c.getCount()];
		int i =0;
		while(c.moveToNext() ) {
			icrIds[i] = c.getString(1);
			i++;
		}
	...
	public List<Form> getForms(String whereClause) {
		SQLiteDatabase db = DatabaseHandler.getInstance(context).getWritableDatabase();
		List<Form> forms = null;
		Cursor c = db.query(DatabaseHandler.TABLE_FORMS, null, whereClause, null, null, null, null/*Form.COLUMN_TIME_STAMP+" ASC"*/);
