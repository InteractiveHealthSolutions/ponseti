...
public class PatientFormParser extends FormParser {
	...
	public void parse(String jsonData, Form form) throws JSONException {
		...
		addAnswer(CreatePatient.QUESTION_ANY_ABNORMALITIES, findValues(abnormalitiesArray));
		addAnswer(CreatePatient.QUESTION_ANY_WEAKNESS, findValues(weaknessArray));
		
	}
	
	public String findValues(JSONArray array) {
		String values = "";
		for(int i=0; i<array.length(); i++) {
			try {
				values += array.getJSONObject(i).getString("value")+";";
